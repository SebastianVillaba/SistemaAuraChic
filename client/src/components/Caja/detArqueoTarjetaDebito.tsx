...
import type { DetArqueoTarjetaDebitoTmp, TarjetaDebito } from '../../types/caja.types';

interface DetArqueoTarjetaDebitoProps {
...
const DetArqueoTarjetaDebito: React.FC<DetArqueoTarjetaDebitoProps> = ({
    idTerminalWeb,
    onTotalChange,
    disabled = false,
}) => {
    const [items, setItems] = useState<DetArqueoTarjetaDebitoTmp[]>([]);
    ...
    const cargarDetalles = async () => {
        setLoading(true);
        try {
            const response = await cajaService.listarDetArqueoTarjetaDebitoTmp(idTerminalWeb);
            if (response.success) {
                setItems(response.result || []);
            }
        } catch (error) {
            console.error('Error al cargar detalles de tarjeta de débito:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAgregar = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            const response = await cajaService.agregarDetArqueoTarjetaDebitoTmp(
                idTerminalWeb,
            ...
            console.error('Error al agregar detalle de tarjeta de débito:', error);
    ...
    const handleEliminar = async (idTarjetaDebito: number) => {
        setSaving(true);
        try {
            const response = await cajaService.eliminarDetArqueoTarjetaDebitoTmp(idTerminalWeb, idTarjetaDebito);
            if (response.success) {
                await cargarDetalles();
            }
        } catch (error: any) {
            console.error('Error al eliminar detalle de tarjeta de débito:', error);
    ...
    const totalTarjetaDebito = items.reduce((sum, item) => sum + item.monto, 0);
    ...
    return (
        <Box>
                    ...
                    <Grid container spacing={2} alignItems="center">
                        <Grid item xs={12} sm={5}>
                            <FormControl fullWidth size="small" required>
                                <InputLabel id="tarjeta-debito-select-label">Tarjeta</InputLabel>
                                <Select
                                    labelId="tarjeta-debito-select-label"
                            ...
                            <TableRow>
                                <TableCell colSpan={disabled ? 2 : 3} align="center" sx={{ py: 3 }}>
                                    <Typography color="text.secondary" variant="body2">
                                        No se han registrado tarjetas de débito.
                                    </Typography>
                                </TableCell>
                            </TableRow>
                        )}
            ...
            <Paper sx={{ p: 1.5, mt: 2, backgroundColor: '#e3f2fd', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant="subtitle2" color="text.secondary">Total Débito:</Typography>
                <Typography variant="h6" fontWeight="bold" color="primary.dark">
                    Gs. {formatCurrency(totalTarjetaDebito)}
                </Typography>
            </Paper>
        </Box>
    );
...
export default DetArqueoTarjetaDebito;
